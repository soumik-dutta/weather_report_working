 ...
 *
 ******************************************************************************/
package com.omoto.configurator.dataStructures;
...
public class BTree<Key extends Comparable<Key>, Value> {
    ...
    public Set<Comparable> getAllKeys() {
        Set<Comparable> listKey = new HashSet<>();
        getAllKeys(root, height, listKey);

        return listKey;
    }

    /**
     * Return all the keys that are there in the BTree
     *
     * @return
     * @author soumik
     * 2017-02-17T11:30:34.242Z
     */
    private void getAllKeys(Node h, int ht, Set<Comparable> hashSet) {
        StringBuilder s = new StringBuilder();
        Entry[] children = h.children;
        if (ht == 0) {
            for (int j = 0; j < h.m; j++) {
                hashSet.add(children[j].key);
            }
        } else {
            for (int j = 0; j < h.m; j++) {
                if (j > 0) {
                ...
                getAllKeys(children[j].next, ht - 1, hashSet);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Set<Comparable> list = st.getAllKeys();

        for (Comparable comparable : list) {
            System.out.print(comparable + " - ");
            System.out.println(st.get(comparable.toString()));
        }
    }
}
